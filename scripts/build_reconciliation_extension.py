...
def build(output):
    ...
    html = html.replace('Сейчас доступны таблицы CSV.', 'Сравниваю таблицы XLSX/CSV/TSV и текстовые документы TXT/DOCX.')
    ...
    manifest = {
        ...
        "version": VERSION,
        "description": "Сравню Excel, CSV, Word и TXT, выделю отличия и помогу подготовить письмо. Обработка внутри браузера.",
        "icons": {"128": "icon.png"},
    ...
    for target in ("firefox", "chrome"):
        ...
        for name in ("transport.js", "worker.mjs", "engine.mjs", "report.mjs", "xlsx-source.mjs", "xlsx-vendor.mjs", "SHEETJS-LICENSE.txt", "text-source.mjs", "text-engine.mjs", "text-report.mjs", "text-zip.mjs", "xml-vendor.mjs", "XMLDOM-LICENSE.txt", "XMLDOM-SOURCE.md", "launcher.html", "launcher.css", "launcher.js", "icon.png"):
