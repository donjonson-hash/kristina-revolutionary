...
def build(output):
    ...
    html = html.replace('Сейчас доступны таблицы CSV.', 'Сейчас доступны таблицы Excel (XLSX), CSV и TSV.')
    ...
    manifest = {
        ...
        "version": VERSION,
        "description": "Сверю Excel, CSV и TSV, покажу ячейки с отличиями и помогу подготовить письмо. Обработка внутри браузера.",
        "icons": {"128": "icon.png"},
    ...
    for target in ("firefox", "chrome"):
        ...
        for name in ("transport.js", "worker.mjs", "engine.mjs", "report.mjs", "xlsx-source.mjs", "xlsx-vendor.mjs", "SHEETJS-LICENSE.txt", "launcher.html", "launcher.css", "launcher.js", "icon.png"):
