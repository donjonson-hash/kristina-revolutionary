...
test('document viewer preserves evidence while navigating distant edits, one-sided text and empty searches', async t => {
  const p = await page(t, 'firefox'), {$} = p;
  const officeHome = $('office-sidebar').parentElement;
  const paragraphs = Array.from({length: 31}, (_, index) => `Раздел ${index + 1}. Срок составляет десять дней.`);
  const removed = 'Прежнее приложение отменяется.';
  const added = '<img src=x onerror=window.viewerInjected=true> Новый пункт.';
  const before = [...paragraphs, removed, 'Подписи сторон.'];
  const after = [...paragraphs.map((text, index) => index === 29 ? text.replace('десять', 'двадцать') : text), 'Подписи сторон.', added];
  await p.file('left', before.join('\n'), 'viewer-a.txt');
  await p.file('right', after.join('\n'), 'viewer-b.txt');
  await p.result();
  const report = await downloadReport(p);
  assert.equal(report.summary.changed, 1);
  assert.equal(report.summary.only_left, 1);
  assert.equal(report.summary.only_right, 1);
  assert.ok(p.dom.window.document.body.classList.contains('review-mode'));
  assert.ok(p.dom.window.document.body.classList.contains('document-mode'));
  assert.ok($('document-scroll').contains($('result-rows')));
  assert.ok($('document-scroll').querySelector('.document-heads'));
  assert.ok($('source-notes-details').contains($('text-source-notes')));
  assert.notEqual($('office-sidebar').parentElement, officeHome);
  assert.ok($('results').contains($('office-panel')));

  const entries = () => [...$('change-list').querySelectorAll('button.change-item')];
  assert.equal(entries().length, 3, 'The sidebar includes changes beyond the current document page');
  assert.match($('change-count').textContent, /3/);
  assert.equal($('page-info').textContent, 'Страница 1 из 2');
  entries()[0].click();
  assert.equal($('page-info').textContent, 'Страница 2 из 2');
  let selected = p.dom.window.document.activeElement;
  assert.equal(selected.dataset.index, '29');
  assert.equal(selected.dataset.category, 'changed');
  assert.ok(selected.classList.contains('active-change'));
  assert.equal(selected.querySelector('.before .text-content').textContent, before[29]);
  assert.equal(selected.querySelector('.after .text-content').textContent, after[29]);
  assert.equal(entries()[0].getAttribute('aria-current'), 'true');
  assert.equal($('change-position').textContent, 'Отличие 1 из 3');
  $('next-change').click();
  selected = p.dom.window.document.activeElement;
  assert.equal(selected.dataset.category, 'only_left');
  assert.equal(selected.querySelector('.before .text-content').textContent, removed);
  assert.ok(selected.querySelector('.after.absent'));
  $('next-change').click();
  selected = p.dom.window.document.activeElement;
  assert.equal(selected.dataset.category, 'only_right');
  assert.equal(selected.querySelector('.after .text-content').textContent, added);
  assert.ok(selected.querySelector('.before.absent'));
  assert.equal($('change-position').textContent, 'Отличие 3 из 3');
  assert.ok($('change-list').textContent.includes('<img'), 'Source snippets remain literal text');
  assert.equal($('results').querySelector('img,script,iframe'), null);
  assert.equal(p.dom.window.viewerInjected, undefined);
  $('previous-change').click();
  assert.equal(p.dom.window.document.activeElement.dataset.category, 'only_left');

  $('document-zoom').value = '130';
  $('document-zoom').dispatchEvent(new p.dom.window.Event('change'));
  assert.ok($('document-scroll').classList.contains('zoom-130'));
  $('document-zoom').value = '85';
  $('document-zoom').dispatchEvent(new p.dom.window.Event('change'));
  assert.ok($('document-scroll').classList.contains('zoom-85'));
  assert.equal($('document-scroll').classList.contains('zoom-130'), false);
  $('search').value = 'Нет такого фрагмента в документах';
  $('search').dispatchEvent(new p.dom.window.Event('input'));
  assert.equal(entries().length, 0);
  assert.equal($('change-empty').hidden, false);
  assert.equal($('next-change').disabled, true);
  assert.equal($('previous-change').disabled, true);
  assert.equal($('result-rows').querySelector('.active-change'), null);
  $('search').value = '';
  $('search').dispatchEvent(new p.dom.window.Event('input'));
  assert.equal(entries().length, 3);
  assert.equal($('change-empty').hidden, true);
  assert.equal($('change-list').querySelector('[aria-current="true"]'), null);
  assert.equal($('change-position').textContent, 'Отличий: 3');
  assert.deepEqual(await downloadReport(p), report, 'Navigation, search and zoom must not alter the exported evidence');

  await p.file('left', Buffer.from('Invalid DOCX content'), 'invalid.docx');
  assert.equal($('results').hidden, true);
  assert.equal(p.dom.window.document.body.classList.contains('review-mode'), false);
  assert.equal(p.dom.window.document.body.classList.contains('document-mode'), false);
  assert.equal(entries().length, 0, 'Replacing a source removes retained change entries');
  assert.equal($('office-sidebar').parentElement, officeHome);
  assert.equal($('results').contains($('office-panel')), false);
});

test('document change list pages independently and keeps selection aligned across filter changes', async t => {
  const p = await page(t, 'chrome'), {$} = p;
  const before = Array.from({length: 31}, (_, index) => `Пункт ${index + 1}: цена 100 рублей.`);
  const after = before.map(text => text.replace('100', '200'));
  await p.file('left', before.join('\n'), 'many-a.txt');
  await p.file('right', after.join('\n'), 'many-b.txt');
  await p.result();
  const report = await downloadReport(p);
  assert.equal(report.summary.changed, 31);
  const entries = () => [...$('change-list').querySelectorAll('button.change-item')];
  assert.equal(entries().length, 25);
  assert.equal($('changes-previous').disabled, true);
  assert.equal($('changes-next').disabled, false);
  $('changes-next').click();
  assert.equal(entries().length, 6);
  assert.equal($('changes-next').disabled, true);
  assert.equal($('changes-previous').disabled, false);
  assert.equal($('changes-page').textContent, '26–31 из 31');
  const last = entries().at(-1);
  assert.equal(last.dataset.index, '30');
  last.click();
  assert.equal($('page-info').textContent, 'Страница 2 из 2');
  assert.equal(p.dom.window.document.activeElement.dataset.index, '30');
  assert.equal($('change-position').textContent, 'Отличие 31 из 31');
  $('next-change').click();
  assert.equal($('page-info').textContent, 'Страница 1 из 2');
  assert.equal(p.dom.window.document.activeElement.dataset.index, '0');
  assert.equal(entries().length, 25);
  assert.equal(entries()[0].getAttribute('aria-current'), 'true');
  assert.equal($('change-position').textContent, 'Отличие 1 из 31');
  $('previous-change').click();
  assert.equal(p.dom.window.document.activeElement.dataset.index, '30');
  assert.equal(entries().at(-1).getAttribute('aria-current'), 'true');

  entries().find(button => button.dataset.index === '29').click();
  assert.equal($('change-position').textContent, 'Отличие 30 из 31');
  $('previous').click();
  assert.equal($('page-info').textContent, 'Страница 1 из 2');
  assert.equal($('change-position').textContent, 'Отличий: 31');
  assert.equal($('change-list').querySelector('[aria-current="true"]'), null);
  assert.equal($('result-rows').querySelector('.active-change'), null);
  $('next-change').click();
  assert.equal(p.dom.window.document.activeElement.dataset.index, '0');
  ...
  assert.equal($('changes-previous').disabled, true);
  assert.equal($('changes-next').disabled, true);
  $('totals').querySelector('[data-category="all"]').click();
  assert.equal(entries().length, 25, 'A changed filter resets the sidebar page');
  assert.equal($('change-list').querySelector('[aria-current="true"]'), null);
  $('search').value = 'Пункт 30:';
  $('search').dispatchEvent(new p.dom.window.Event('input'));
  assert.equal(entries().length, 1);
  assert.equal(entries()[0].dataset.index, '0', 'Filtered navigation indexes the visible records');
  entries()[0].click();
  assert.equal(p.dom.window.document.activeElement.querySelector('.before .text-content').textContent, before[29]);
  assert.equal($('change-position').textContent, 'Отличие 1 из 1');
  assert.deepEqual(await downloadReport(p), report);
});
